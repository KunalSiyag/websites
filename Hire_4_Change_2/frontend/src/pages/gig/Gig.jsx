...
function Gig() {
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        const response = await fetch(`${baseUrl}/fetch-job/${id}`);
  ...
  const applyForJob = async () => {
    try {
      const response = await fetch(`${baseUrl}/apply-job`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ applicantUsername: userId, jobID: id }),
      });

      if (!response.ok) throw new Error("You have already applied or job is completed");
  ...
  const completedJob = async (jobId, applicantUsername) => {
    try {
      const response = await fetch(`${baseUrl}/complete-job`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ jobId: jobId, applicantUsername: applicantUsername }),
      });
    ...
    <div className="container mx-auto p-6">
      <div className="flex flex-col md:flex-row gap-8">
        <div className="w-full md:w-2/3">
          <h1 className="text-6xl font-bold mb-4 text-gray-800">{data.title}</h1>

          <div className="flex justify-between items-center mb-12">
            <div className="flex items-center">
              <img className="w-16 h-16 rounded-full border border-gray-300" src={logo} alt="User" />
              <span className="text-lg font-semibold text-gray-700">{data.publisher}</span>
            </div>
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/messages", { state: { userId: userId, receiver: data.publisher } })}>
              <h1 className="text-xl font-bold pr-1">chat</h1>
              <i className="ri-chat-3-line ri-2x"></i>
            </div>
          </div>

          <h2 className="text-2xl font-semibold mb-2 text-gray-800">About This Gig</h2>
          <p className="text-gray-700 mb-6">{data.description}</p>

          <h2 className="text-2xl font-semibold mb-2 text-gray-800">Job Details</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white border-gray-300 rounded-lg shadow-lg">
              <tbody>
                ...
                <tr className="border-b border-gray-200">
                  <td className="py-2 px-4 text-gray-600">Estimated Duration</td>
                  <td className="py-2 px-4 text-gray-800">{data.estimatedDuration}</td>
                </tr>
                <tr className="border-b border-gray-200">
                  <td className="py-2 px-4 text-gray-600">Skills Required</td>
                  <td className="py-2 px-4 text-gray-800">{data.skillsRequired.join(", ")}</td>
                </tr>
                <tr className="border-b border-gray-200">
        ...
        <div className="w-full md:w-1/3 bg-gray-100 p-4 rounded-lg shadow-lg">
          ...
          <div className="flex flex-row gap-2">
            {data.keywords.map((keyword, index) => (
              <div key={index} className="bg-gray-200 p-2 px-3 opacity-65 font-semibold rounded-full flex justify-center items-center text-nowrap text-gray-700">
                {keyword}
              </div>
            ))}
          </div>
              ...
              <ul>
                {applicants.length === 0 ? (
                  ...
                  applicants.map((applicant, index) => (
                    <li key={index} className="mb-2 flex justify-between items-center text-gray-800">
                      {applicant}
